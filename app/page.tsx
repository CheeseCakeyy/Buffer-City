...
export default function Home() {
 ...
 return <main>
 <header><div className="brand"><div><h1>ascii city<span> / maple street</span></h1><p>An inhabited drawing.</p></div></div><div className="header-right"><button onClick={()=>setPanel(!panel)}>How it works ↗</button></div></header>
 <section className="world" aria-label="Explorable ASCII city">
 <canvas ref={canvas} tabIndex={0} aria-label="Click a street to walk there. WASD or arrows to walk, Shift to run, drag to orbit, Q and E to rotate. Scroll to zoom."/>
 
 <div className="clock"><strong>{stats.time}</strong></div>
 <div className="legend">Click a street to walk · drag to look around</div>
 <div className="camera-controls"><button className={overview?"active":""} onClick={()=>{engine.current?.setOverview();setOverview(!overview);}}>{overview?"Street view":"Whole block"}</button><button aria-label="Rotate left" onClick={()=>engine.current?.rotate(-1)}>↶</button><button aria-label="Rotate right" onClick={()=>engine.current?.rotate(1)}>↷</button><button aria-label="Zoom out" onClick={()=>engine.current?.zoomBy(1.15)}>−</button><button aria-label="Zoom in" onClick={()=>engine.current?.zoomBy(.87)}>+</button><button onClick={()=>{engine.current?.reset();setOverview(false);}}>Find me</button></div>
 <div className="inspection"><p>{stats.selected}</p></div>
 ...
 <li><h3>02 / Place an orthographic camera</h3><p>Street view looks down at 27°, with a closer camera that follows you smoothly. Whole block switches to a 35.3° overview. Parallel rays keep far buildings the same size as near ones. Q / E rotates the view; zoom changes the area each cell covers.</p></li>
 ...
 <li><h3>05 / Draw, then repeat</h3><p>The canvas draws the surface glyphs, then projects window frames, roof rails, signs and curb lines into the same character grid. Each line sample checks its depth against the ray buffer, so it disappears behind nearer walls. This is CPU raycasting, not a 3D image passed through a text filter. This version has no secondary reflection rays or cast-shadow rays.</p></li></ol>
 ...
 <p>Clicking a street casts one picking ray, then searches a half-unit walkable grid using breadth-first search. You follow the route around buildings; WASD cancels it. Dragging orbits the camera. The amber player marker deliberately stays visible through walls so you never lose your position.</p></aside>}
 </section>
 <footer><div><span className="key">W A S D</span> Walk <span className="key">SHIFT</span> Run <span className="key">Q E</span> Orbit <span className="key">SCROLL</span> Zoom</div><div className="footer-actions"><span>{stats.cells.toLocaleString()} rays / frame · {stats.fps} fps</span><button onClick={()=>{const next=!paused;setPaused(next);if(engine.current)engine.current.paused=next;}}>{paused?'▶ Resume city':'Ⅱ Pause city'}</button><button onClick={()=>engine.current?.toggleNight()}>Day / night</button></div></footer>
 </main>;
}
