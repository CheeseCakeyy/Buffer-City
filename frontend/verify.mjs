...
const { inscriptionLines } = await import(load('lib/slate-inscription.ts'));
assert.deepEqual(inscriptionLines('Adwait'), ['Adwait']);
assert.deepEqual(inscriptionLines('Adwait Tagalpallewar'), ['Adwait', 'Tagalpallewar']);
assert.equal(inscriptionLines('ABCDEFGHIJKLMNOPQRSTUVWX').join(''), 'ABCDEFGHIJKLMNOPQRSTUVWX');
assert.equal(inscriptionLines('李 明').join(' '), '李 明');
...
const inscriptionCalls = [];
renderer.player = [0, 0, 82]; renderer.focus = [0, 0, 86]; renderer.angle = .49;
renderer.setPOV('third');
renderer.setVisitorSlates([{ ...testSlates[0], name: 'Adwait Tagalpallewar' }]);
renderer.simulate(0); renderer.camera(); renderer.buildRowBounds();
renderer.depths.fill(Infinity);
const previousFillText = renderer.ctx.fillText;
renderer.ctx.fillText = (text, x, y) => inscriptionCalls.push({ text, x, y, align: renderer.ctx.textAlign, baseline: renderer.ctx.textBaseline });
renderer.drawSlateInscriptions(false);
assert.deepEqual(inscriptionCalls.map(call => call.text), ['Adwait', 'Tagalpallewar']);
assert.ok(inscriptionCalls.every(call => call.x === 0 && call.align === 'center' && call.baseline === 'middle'));
assert.ok(Math.abs(inscriptionCalls[0].y + inscriptionCalls[1].y) < 1e-8, 'Wrapped names must be vertically centered');
renderer.ctx.fillText = previousFillText;
renderer.setVisitorSlates([]);
console.log('Full Unicode slate names, balanced wrapping and horizontal/vertical inscription centering passed.');
