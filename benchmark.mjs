...
const load = cityModuleLoader(reference);
const { City, buildings } = await import(load('lib/city.ts'));
const scenarios = [
  { name: 'crossing', player: [0, 0, 18], angle: 0.49 },
  { name: 'shopfront', player: [-1, 0, -5], angle: 1.57 },
  { name: 'vehicles', player: [-7, 0, 18], angle: Math.PI },
  ...(!reference ? [
    { name: 'park', player: [-42, 0, 16], angle: .65 },
    { name: 'construction', player: [42, 0, -24], angle: .25 },
    { name: 'market', player: [0, 0, -25], angle: .15 },
  ] : []),
];
const output = [];
const median = a => [...a].sort((a, b) => a - b)[Math.floor(a.length / 2)];
for (const pov of ['third', 'first', 'second', ...(!reference ? ['overview'] : [])]) for (const scene of scenarios) {
  ...
  Object.assign(r, { player: [...scene.player], focus: [...scene.player], angle: scene.angle,
    lookPitch: 0.04, followPitch: 0.24, followDistance: 5.5, fov: 72, span: pov === 'overview' ? 195 : 45,
    ...
    overview: pov === 'overview', pov: pov === 'overview' ? 'third' : pov, mode: 'ink', ctx, canvas: { dataset: {} },
  ...
  const row = { pov, scene: scene.name, cpuMs: +median(samples).toFixed(2), objects: r.objects.length, visible: r.visibleObjects?.size ?? 0 };
...
console.table(output.map(({ imageHash: _imageHash, ...row }) => row));
